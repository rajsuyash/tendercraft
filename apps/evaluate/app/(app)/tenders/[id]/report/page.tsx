...
export default async function ReportPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const res = await engineJson<Report>(`/api/tenders/${id}/report`);
  ...
  return (
    <main className="mx-auto max-w-4xl px-page py-page">
      <Link href={`/tenders/${id}`} className="text-sm text-primary hover:underline print:hidden">← Evaluation</Link>
